  ...
  (let ((map (make-sparse-keymap))
    ...
    (with-temp-buffer
      ...
      (while (re-search-forward "^<Multi_key> \\([^:]+\\): \"\\([^\"]+\\)\"" nil t)
        ;; (when (> (length (match-string 2)) 1)
        ;;   (push (match-string 2) errors))
        (let* ((to-char (string-to-char (match-string 2)))
          ...
          (if (memq -1 from-chars)
              (push from-keys errors)
            (push (cons (apply 'vector from-chars) (vector to-char))
                  x-compose-char-map)))))
    ;; (insert (pp-to-string errors))
    (insert (pp-to-string x-compose-char-map))))
