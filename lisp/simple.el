...
(defvar minibuffer-local-shell-command-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map [M-left]   #'minibuffer-previous-completion)
    (define-key map [M-right]  #'minibuffer-next-completion)
    (define-key map [M-up]     #'minibuffer-previous-line-completion)
    (define-key map [M-down]   #'minibuffer-next-line-completion)
